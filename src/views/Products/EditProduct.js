...
handleEditedItem=(event)=>{
  ...
  Adapter.fetchRequest(url, submissionBody, "PATCH").then(() => {
    ...
    this.props.allProducts[this.props.allProducts.indexOf(this.props.product)].category= this.state.category
...
<div style={{padding:'20px'}}>
  <Link style={{ color:'black'}} to="/items">Go back</Link>
  ...
  <form className='new-item-form' onSubmit={this.handleEditedItem}>
      <h2 style={{textAlign:'center', marginBottom:'20px'}}>Edit Item From</h2>
      <div className="input-div">
        <label className="left-label" >Item Name</label>
        <input className="right-label" type="text" value={this.state.item_name} onChange={this.newItem_Name} required/>
      </div>
      <div className="input-div">
        <label className="left-label" >Retail Price</label>
        <input className="right-label" type="number" value={this.state.retail_price} step="1" onChange={this.newRetailPrice} required/>
      </div>
      <div className="input-div">
        <label className="left-label" >Promo Price</label>
        <input className="right-label" type="number" value={this.state.pomo_price} step="1" onChange={this.newPomoPrice} required/>
      </div>
      <div className="input-div">
        <label className="left-label" >Last Cost</label>
        <input className="right-label" value={this.state.last_cost} step="1" onChange={this.newLastCost}required/>
      </div>
      <div className="input-div">
        <label className="left-label" >Most Recent Vendor</label>
        <input className="right-label" type="text"  value={this.state.most_recent_vendor} onChange={this.newMostRecentVendor}required/>
      </div>
      <div className="input-div">
        <label className="left-label" >Unit</label>
        <input className="right-label" type="text"  value={this.state.unit} onChange={this.newProductUnit} required/>
      </div>
      <div className="input-div">
        <label className="left-label" >Status</label>
        <input className="right-label" type="text"  value={this.state.status} onChange={this.newStatus} required/>
      </div>
      <div className="input-div">
        <label className="left-label" >Forecast Sales For The Next 3 Months</label>
        <input className="right-label" type="text"  value={this.state.forecast_sales_three_months} onChange={this.newForecast}/>
      </div>
      <div className="input-div">
        <label className="left-label" >Need To Order For The Next 3 Months</label>
        <input className="right-label" type="text"  value={this.state.need_to_order_for_next_three_months} onChange={this.newNeedToOrder}/>
      </div>
      <div className="input-div">
        <label className="left-label" >Annualized Sales</label>
        <input className="right-label" type="text"  value={this.state.annualized_sales} onChange={this.newAnnualizedSales}/>
      </div>
      <div className="input-div">
        <label className="left-label" >Annualized QTY</label>
        <input className="right-label" type="text"  value={this.state.annualized_qty} onChange={this.newAnnualizedQty}/>
      </div>
      <div className="input-div">
        <label className="left-label" >Upload Product Image</label>
        <input  type="file" style={{width:'30%'}} onChange={this.newImage_url}/>
      <button onClick={this.uploadHandler} className="button file-upload-button">Click to Upload</button>
      </div>
      <div className="input-div">
        <label className="left-label" >Category</label>
        <input className="right-label" type="text"  value={this.state.category} onChange={this.newCategory} required/>
      </div>
      <div className="input-div">
        <label className="left-label" >Barcode</label>
        <input className="right-label" type="text"  value={this.state.barcode} onChange={this.newBarcode} required/>
      </div>
      <button type="submit" style={{marginTop:'30px'}}>Update Item</button>
    </form>
    <div style={{display:'flex'}}>
      <img id="preview" src={this.state.image_url} style={{margin:'20px auto'}} height="222" width="332" alt="Please Click Upload"/>
    </div>
</div>)
 }
 }
