   ...
   processReceiving=()=> {
     ...
     this.props.allOrders[this.props.allOrders.indexOf(this.props.order)].updated_at=date
       ...
       Adapter.fetchRequest(url,submissionBody,"PATCH").then(()=>{
         const productUrl="http://localhost:3000/items/"+this.props.order.product_id
   ...
   return(
    <div className="order-card">
      <div className='order-card-left' id="center-order-card">
        <h3 style={{margin:'10px 0px', textTransform:'uppercase'}}>{this.props.order.product_name}</h3>
        <p style={{margin:'10px 0px'}}>Quantity: {this.props.order.qty}</p>
        <p style={{margin:'10px 0px'}}>Price: {this.props.order.price}</p>
        <p style={{margin:'10px 0px'}}>Total Dollars: {this.props.order.total_dollars}</p>
        <p style={{margin:'10px 0px'}}>Vendor Name: {this.props.order.vendor_name}</p>
        <p style={{margin:'10px 0px'}}>Order By: {this.props.order.order_by}</p>
        <p style={{margin:'10px 0px'}}>Received By: {this.props.order.received_by}</p>
        <p style={{margin:'10px 0px'}}>Order Created Date: {this.props.order.created_at.slice(0,10)}</p>
        {this.props.order.received ? <h4 style={{marginTop:'5px'}}>Received Already!</h4> : <div><button style={{marginTop:'20px'}} className="ui teal button" onClick={this.processReceiving}>Click To Confirm Product Received</button></div>}
      </div>
